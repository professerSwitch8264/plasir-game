...
export class CharacterCreator extends Scene {
    ...
    saveHistory() {
        ...
        let currentFrame =
            this.animsData[this.currentAnim][this.currentFrameIdx];
    ...
    create() {
        ...
        if (savedCol) {
            try {
                this.collectionDB = JSON.parse(savedCol);
            } catch (e) {
                this.collectionDB = {};
            }
        } else {
        ...
        this.add.grid(
            540,
            this.startY + (this.gridSize * this.cellSize) / 2,
        ...
        this.selection.rect = this.add
            .rectangle(0, 0, 0, 0, 0x00ff00, 0.2)
        ...
        drawZone.on("pointerdown", (pointer) => {
            this.handlePointerDown(pointer);
        });
        drawZone.on("pointermove", (pointer) => {
    ...
    handlePointerDown(pointer) {
        ...
        if (this.currentTool === this.tools.SELECT) {
            if (this.selection.active) {
                ...
                if (
                    x >= this.selection.bufferX &&
                    x <
                        this.selection.bufferX +
                            this.selection.buffer[0].length &&
                    y >= this.selection.bufferY &&
                    y < this.selection.bufferY + this.selection.buffer.length
                ) {
                    ...
                    if (!this.selection.isLifted) {
                        ...
                        let frame =
                            this.animsData[this.currentAnim][
                                this.currentFrameIdx
                            ];
                        for (
                            let by = 0;
                            by < this.selection.buffer.length;
                            by++
                        ) {
                            for (
                                let bx = 0;
                                bx < this.selection.buffer[0].length;
                                bx++
                            ) {
    ...
    updateSelectionVisuals() {
        if (this.selection.isSelecting) {
            ...
            this.selection.rect.setPosition(
                this.startX + x1 * this.cellSize,
                this.startY + y1 * this.cellSize,
            );
            this.selection.rect.setSize(
                (x2 - x1 + 1) * this.cellSize,
                (y2 - y1 + 1) * this.cellSize,
            );
            this.selection.rect.setVisible(true);
        } else if (this.selection.active) {
            this.selection.rect.setPosition(
                this.startX + this.selection.bufferX * this.cellSize,
                this.startY + this.selection.bufferY * this.cellSize,
            );
                    ...
                    for (let x = 0; x < this.selection.buffer[0].length; x++) {
                        ...
                        if (color !== null) {
                            let rect = this.add
                                .rectangle(
                                    this.startX +
                                        (this.selection.bufferX + x) *
                                            this.cellSize +
                                        1,
                                    this.startY +
                                        (this.selection.bufferY + y) *
                                            this.cellSize +
                                        1,
                                    this.cellSize - 2,
                                    this.cellSize - 2,
    ...
    createToolUI() {
        ...
        toolNames.forEach((name, i) => {
            let btn = this.add
                .rectangle(
                    ...
                    60,
                    40,
                    this.currentTool === i ? 0xffcc00 : 0x555555,
                )
                .setInteractive();
            let txt = this.add
                .text(toolX, startY + i * 55, name, {
                    fill: this.currentTool === i ? "#000" : "#fff",
                })
                .setOrigin(0.5);
            btn.on("pointerdown", () => {
                if (
                ...
                this.toolBtns.forEach((b, j) => {
                    b.btn.setFillStyle(
                        this.currentTool === j ? 0xffcc00 : 0x555555,
                    );
        ...
        const addTopBtn = (tx, lbl, color, cb) => {
            let b = this.add
                .rectangle(tx, topY, 70, 30, color)
                .setInteractive();
            this.add
                .text(tx, topY, lbl, { fill: "#fff", fontSize: "14px" })
                .setOrigin(0.5);
            b.on("pointerdown", cb);
        };
        ...
        addTopBtn(this.startX + 200, "Copy", 0xaaaa00, () => {
            if (this.selection.active && this.selection.buffer) {
                this.clipboard = JSON.parse(
                    JSON.stringify(this.selection.buffer),
                );
            }
        });
        addTopBtn(this.startX + 280, "Paste", 0xaaaa00, () => {
            if (this.clipboard) {
                ...
                this.selection.buffer = JSON.parse(
        ...
        let onionBtn = this.add
            .rectangle(
                this.startX + 450,
                topY,
                100,
                30,
                this.onionSkinEnabled ? 0x00aa00 : 0x555555,
            )
            .setInteractive();
        let onionTxt = this.add
            .text(this.startX + 450, topY, "Onion Skin", {
                fill: "#fff",
                fontSize: "14px",
            })
            .setOrigin(0.5);
    ...
    createAnimUI() {
        ...
        this.animContainer = this.add.container(
            this.startX + this.gridSize * this.cellSize + 80,
            150,
        );

        // Animation Tabs
        const btnIdle = this.add
            .rectangle(
                0,
                0,
                80,
                40,
                this.currentAnim === "idle" ? 0x00ffcc : 0x555555,
            )
            .setInteractive();
        const txtIdle = this.add
            .text(0, 0, "IDLE", {
                fill: this.currentAnim === "idle" ? "#000" : "#fff",
            })
            .setOrigin(0.5);
        btnIdle.on("pointerdown", () => {
            this.applySelection();
            this.currentAnim = "idle";
            this.currentFrameIdx = 0;
            this.history = [];
        ...
        const btnWalk = this.add
            .rectangle(
                90,
                0,
                80,
                40,
                this.currentAnim === "walk" ? 0x00ffcc : 0x555555,
            )
            .setInteractive();
        const txtWalk = this.add
            .text(90, 0, "WALK", {
                fill: this.currentAnim === "walk" ? "#000" : "#fff",
            })
            .setOrigin(0.5);
        btnWalk.on("pointerdown", () => {
            this.applySelection();
        ...
        frames.forEach((f, i) => {
            const fBtn = this.add
                .rectangle(
                    ...
                    170,
                    40,
                    this.currentFrameIdx === i ? 0xffcc00 : 0x444444,
                )
                .setInteractive();
            const fTxt = this.add
                .text(45, 60 + i * 50, `Frame ${i + 1}`, {
                    fill: this.currentFrameIdx === i ? "#000" : "#fff",
                })
                .setOrigin(0.5);
            fBtn.on("pointerdown", () => {
                this.applySelection();
        ...
        // Add Frame Button
        const addBtnY = 60 + frames.length * 50;
        const addBtn = this.add
            .rectangle(45, addBtnY, 170, 40, 0x00aa00)
            .setInteractive();
        const addTxt = this.add
            .text(45, addBtnY, "+ Add Frame", { fill: "#fff" })
            .setOrigin(0.5);
        ...
        // Delete Frame Button
        const delBtnY = addBtnY + 50;
        const canDelete = frames.length > 1;
        const delBtn = this.add
            .rectangle(45, delBtnY, 170, 40, canDelete ? 0xaa2222 : 0x555555)
            .setInteractive();
        const delTxt = this.add
            .text(45, delBtnY, "Delete Frame", { fill: "#fff" })
            .setOrigin(0.5);

        delBtn.on("pointerdown", () => {
            if (this.animsData[this.currentAnim].length <= 1) return;
            this.applySelection();
            this.animsData[this.currentAnim].splice(this.currentFrameIdx, 1);
            this.currentFrameIdx = Math.min(
                this.currentFrameIdx,
                this.animsData[this.currentAnim].length - 1,
            );
            this.history = [];
            this.redoStack = [];
            this.clearSelection();
        ...
        this.animContainer.add([addBtn, addTxt, delBtn, delTxt]);
    }
    ...
    createBottomUI() {
        ...
        this.add
            .text(this.startX, paletteY, "Brush Color:", {
                fontSize: "20px",
                fill: "#fff",
            })
            .setOrigin(0, 0.5);
        ...
        let colorDom = this.add
            .dom(this.startX + 140, paletteY)
            .createFromHTML(colorInputHTML);
        ...
        const playBtn = this.add
            .rectangle(540, paletteY + 120, 250, 60, 0x00aa00)
            .setInteractive();
        playBtn.on("pointerdown", () => this.saveAndPlay());
        this.add
            .text(540, paletteY + 120, "Save & Play", {
                fontSize: "28px",
                fill: "#ffffff",
                fontStyle: "bold",
            })
        ...
        setTimeout(() => {
            ...
            document
                .getElementById("btnSaveSkin")
                ?.addEventListener("click", () => {
                    let name = document.getElementById("charName").value.trim();
                    if (!name) return alert("Please enter a skin name first!");
                    this.collectionDB[name] = JSON.parse(
                        JSON.stringify(this.animsData),
                    );
                    localStorage.setItem(
                        "pixelArtCollection",
                        JSON.stringify(this.collectionDB),
                    );
                    this.updateCollectionDropdown();
            ...
            document
                ...
                ?.addEventListener("change", (e) => {
                    ...
                    if (name && this.collectionDB[name]) {
                        this.saveHistory();
                        this.animsData = JSON.parse(
                            JSON.stringify(this.collectionDB[name]),
                        );
            ...
            document
                ...
                ?.addEventListener("click", () => {
                    let dataStr =
                        ...
                        encodeURIComponent(JSON.stringify(this.collectionDB));
    ...
    renderCurrentFrame() {
        let currentData =
            this.animsData[this.currentAnim][this.currentFrameIdx];
        let prevData =
            this.onionSkinEnabled && this.currentFrameIdx > 0
                ? this.animsData[this.currentAnim][this.currentFrameIdx - 1]
                : null;
    ...
    saveAndPlay() {
                ...
                for (let y = 0; y < this.gridSize; y++) {
                    for (let x = 0; x < this.gridSize; x++) {
                        if (frameData[y][x] !== null) {
                            let hexColor =
                                "#" +
                                frameData[y][x].toString(16).padStart(6, "0");
        ...
        if (this.textures.exists("customPlayer"))
            this.textures.remove("customPlayer");
        ...
        this.scene.start("Game", {
            playerName: charName,
            idleCount: idleFrames,
            walkCount: walkFrames,
        });
    }
}
